...
// @match        https://trade.aliexpress.com/*
...
(() => {
'use strict';
const KEY='AE_EXPORT_SK_2026', PANEL='ae-export-sk-panel', SEP=';';
...
function money(s){s=clean(s);let currency=/€|EUR/i.test(s)?'EUR':/US\s*\$|USD|\$/i.test(s)?'USD':/£|GBP/i.test(s)?'GBP':'';const a=[...s.matchAll(/(?:US\s*\$|\$|€|EUR|USD|£|GBP)?\s*([0-9]+(?:[.,][0-9]{1,2})?)/gi)];return {value:a.length?a.at(-1)[1].replace(',','.'):'',currency}}
...
function clear(){if(confirm('Vymazať nazbierané exportné údaje?')){localStorage.removeItem(KEY);count();setStatus('Údaje vymazané.')}}
...
function btn(p,s,f,c){const b=document.createElement('button');b.textContent=s;b.style.cssText=`width:100%;margin:4px 0;padding:7px;border:0;border-radius:6px;color:white;background:${c};cursor:pointer`;b.onclick=f;p.append(b)}
function panel(){if(document.getElementById(PANEL))return;const p=document.createElement('div');p.id=PANEL;p.style.cssText='position:fixed;top:90px;right:12px;width:245px;z-index:2147483647;background:#18181c;color:white;border:1px solid #555;border-radius:10px;padding:10px;font:12px Arial;box-shadow:0 4px 20px #0006';p.innerHTML='<b>AliExpress export SK 2026</b><br>Produktové riadky: <span id="ae-count">0</span><div style="height:6px"></div>';btn(p,'1. Naskenovať túto stránku',()=>scan(true),'#238636');btn(p,'2. Export CSV (Excel)',exportCSV,'#1f6feb');btn(p,'Export JSON (odporúčané)',exportJSON,'#8250df');btn(p,'Kopírovať CSV',copy,'#0969da');btn(p,'Vymazať uložené dáta',clear,'#b62324');const s=document.createElement('div');s.id='ae-status';s.style.cssText='margin-top:8px;color:#d7ffd7;line-height:1.35';s.textContent='Nejasné údaje sa nedohadujú; zostanú prázdne a surový text ostáva na kontrolu.';p.append(s);document.body.append(p);count()}
setTimeout(panel,1200);new MutationObserver(()=>setTimeout(panel,300)).observe(document.documentElement,{childList:true,subtree:true});
})();
